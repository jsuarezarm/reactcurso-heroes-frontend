  ...
  return (
    <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
      <div className="relative h-64">
        ...
        {/* Universe badge */}
        <Badge className={`absolute top-3 right-3 text-xs text-white ${hero.universe === 'DC' ? 'bg-blue-600' : 'bg-red-600'}`}>
          {hero.universe}
        </Badge>
          ...
          <div className="flex flex-wrap gap-1">
            {
              hero.powers.slice(0, 3).map(power => (
                <Badge key={power} variant="outline" className="text-xs">{power}</Badge>
              ))
            }
            {
              hero.powers.length > 3 &&
              <Badge variant="outline" className="text-xs bg-gray-100">+{hero.powers.length - 3}</Badge>
            }
          </div>
        </div>
